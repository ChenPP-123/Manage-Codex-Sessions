...
`mcs` 是一个用于批量管理 Codex CLI 会话的终端工具。它分别显示未归档和已归档会话，并支持多选归档、取消归档或永久删除。

## 功能简介

- 按 `Tab` 在未归档和已归档会话之间切换
- 展示会话名称、项目目录和会话记录的 Git 分支
- 使用键盘移动焦点并跨列多选
- 批量归档未归档会话、取消归档已归档会话，或删除任意会话
...
程序会读取 Codex CLI 会话，并一次显示一个会话视图。按 `Tab` 可在未归档和已归档会话之间切换。MCS 使用 Codex 官方 App Server 接口，不会直接修改 `~/.codex` 数据库或会话文件。
...
| `Tab` | 切换未归档和已归档会话 |
| `Space` | 选择或取消选择 |
| `r` | 在未归档视图重命名当前焦点会话；输入名称后按 `Enter` 确认，按 `Esc` 取消 |
| `a` | 归档选中的未归档会话 |
| `u` | 在已归档视图取消归档选中的会话 |
| `d` | 删除选中的会话 |
| `q` | 退出 |

选中的会话以 `[x]` 标识，可同时选择多条。按 `a` 时，已归档会话会被跳过；按 `u` 时，未归档会话会被跳过。已归档视图不提供重命名，以避免 Codex App Server 无法定位部分归档会话记录。按 `d` 后必须再按 `y` 确认，按 `n` 取消。
